...
module.exports = async function (context, req) {
    ...
    try {
        ...
        context.log('Parsed data:', { patakaId, itemCount: items?.length, hasPhoto: !!photoFile, isTest });
        ...
        if (photoFile && photoFile.data && photoFile.data.length > 0) {
            try {
                ...
                const containerName = isTest ? 'taking-photos-test' : 'taking-photos';
                ...
                const blobName = `takings/${patakaId}/${timestamp}_${randomStr}.${ext}`;
                ...
                // Continue without photo rather than failing entire taking
            }
        }
        ...
        context.log('Taking successful:', transactionIds.length, 'items processed');

        // Success response
        context.res.status = 200;
        context.res.body = {
            ok: true,
            message: 'Taking submitted successfully',
            userId: finalUserId,
        ...
        context.log.error('Error in SubmitTaking:', error);
